...
if uploaded_file:
    ...
    with tab2:
        ...
        fig_pred = px.line(pd.concat([df_dia, df_pred]), x="Fecha", y="Cantidad", title="Predicción de ventas para la próxima semana")
    ...
    with tab3:
        st.subheader("🤖 Simulación IA: Precio óptimo por producto")
        for producto in productos:
            df_p = df[df["Producto"] == producto]
            if len(df_p) < 5:
                continue
            ...
            st.slider("Ajusta el precio", 
                      min_value=float(precios.min()*0.5), 
                      max_value=float(precios.max()*1.5), 
                      value=float(precio_optimo), 
                      step=0.05, 
                      key=producto)
